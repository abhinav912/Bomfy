...
st.markdown("<h1 style='text-align: center; color:;'>BOMFY</h1>", unsafe_allow_html=True)
st.markdown("<h2 style='text-align: center; color: grey;'>Bring Out Most From Youtube</h2>", unsafe_allow_html=True)
st.write("---")
...
if selected == "Analyze Video":
    ...
    with col3:
        st.write("")
    video_link=st.text_input('Paste Link of Youtube Video here :',placeholder='https://www.youtube.com/watch?v=r-GFmH0EK9Y')
# if st.button('Analyse Video'):
    ...
    #    st.download_button('Download Zip', f, file_name='archive.')
        ...
        # # icon("search")
        # selected = st.text_input("", "Analyze Video...")
...
if selected == "About":
    with st.container():
        ...
        st.write("---")
        st.markdown("<h2 style='text-align: center; color: ;'>Our Team 🖊️</h2>", unsafe_allow_html=True)
        st.write("##")
        col1, col2, col3, col4 = st.columns(4)
        ng = Image.open("NamanGarg.jpg")
        with col1:
            st.image(ng)
            st.write("Naman Garg")
            st.write("102083045")
...
if selected == "Search Topic":
    with st.container():
        ...
        # st.write(x)
            ...
            st.write(video_data.sort_values(by = 'score',ascending=False,ignore_index=True))
